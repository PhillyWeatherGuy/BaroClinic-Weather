...
function decompressLevel2(arrayBuffer) {
    ...
    if (decompressedChunks.length === 0) {
        // Fallback: decode as standalone BZIP2
        try {
...
function processVolume(rawBytes, radarLat, radarLon, bounds, targetTiltIndex = 0) {
    ...
    while (offset + 64 <= rawBytes.length) {
        // Check for Message 31 (Byte 3 of Message Header = 31)
        let msgPos = -1;
        if (view.getUint8(offset + 3) === 31) {
            msgPos = offset;
        } else if (offset + 12 < rawBytes.length && view.getUint8(offset + 15) === 31) {
            msgPos = offset + 12;
        }

        if (msgPos !== -1 && msgPos + 48 <= rawBytes.length) {
            // Data Header Block starts at msgPos + 16 (Table XVII-A)
            const hdrPos = msgPos + 16;
            ...
            // Find Reflectivity ("REF" or "DREF") block pointer
            let refOffset = -1;
            ...
            if (refOffset > 0 && refOffset + 28 <= rawBytes.length) {
                ...
                for (let g = 0; g < availableGates; g++) {
                    ...
                    // Skip 0 (below SNR) and 1 (range folded)
                    if (rawVal <= 1) continue;

                    // Calculate real dBZ
                    const dbz = (rawVal - offsetVal) / scale;
                    if (dbz < 10.0) continue; // Clear air threshold

                    // Map to standard 0..255 byte scale matching Level 3 palette
                    ...
                    // Beam thickness vertical splat to eliminate conical gaps
        ...
        offset += (offset + 2432 <= rawBytes.length) ? 2432 : 64;
    }
    ...
    console.log(`📡 [Level 2 Worker] Integrated ${gateCountProcessed} storm gates into 3D voxel grid.`);
